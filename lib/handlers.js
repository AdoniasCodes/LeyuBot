...
function attachHandlers(bot) {
  ...
  bot.onText(/\/start/, (msg) => {
    ...
    bot.sendMessage(chatId, messages[lang].welcome(userName), getMenuOptions(lang));
  });
  ...
  bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data === 'language_picker') {
      ...
      await bot.sendMessage(chatId, messages[lang].support, { parse_mode: 'Markdown' });
    } else if (data === 'show_tutorial') {
      await bot.sendMessage(chatId, messages[lang].tutorial, { parse_mode: 'Markdown' });
    }
  });
